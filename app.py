...
def split_xml(tree, max_unique, outdir, dry_run=False, verbose=False):
    ...
    for item in root.findall("ITEM"):
        ...
        if key not in merged:
            ...
            q1 = int(merged[key].findtext("QTY", default="0"))
            q2 = int(item.findtext("QTY", default="0"))
            merged[key].find("QTY").text = str(q1 + q2)
...
def split_xml_by_color(tree, max_unique, outdir, dry_run=False, verbose=False):
    """
    Split XML inventory by color, then chunk each color by max_unique.
    """
    root = tree.getroot()
    by_color = defaultdict(list)

    # Group items by (part, color)
    merged = {}
    for item in root.findall("ITEM"):
        part = item.findtext("ITEMID", default="?")
        color = item.findtext("COLOR", default="?")
    ...
    # Distribute merged entries by color group
    for (part, color), item in merged.items():
        by_color[color].append(item)

    if verbose:
        Log.info(f"Detected {len(by_color)} colors for split by color.")
    ...
    # Now split each color list
    for color, items in by_color.items():
        chunks = [items[i:i + max_unique] for i in range(0, len(items), max_unique)]
        if verbose:
            Log.info(f"[{color}] {len(items)} unique items → {len(chunks)} files")
        ...
        for idx, chunk in enumerate(chunks, start=1):
            ...
            outname = f"output_{color}_{idx}.xml"
...
def main():
    ...
    # init
    sub.add_parser("init", help="Create the default assets folder structure")
    ...
    # split
    p_split = sub.add_parser("split", help="Split an XML into chunks")
    p_split.add_argument("--input", required=True)
    p_split.add_argument("--max", type=int, default=1000)
    p_split.add_argument("--bycolor", action="store_true",
                         help="Split separately for each color")
    ...
    if args.cmd == "split":
        ...
        if args.bycolor:
            outputs = split_xml_by_color(tree, args.max, outdir,
                                         dry_run=args.dry_run,
                                         verbose=args.verbose)
        else:
            outputs = split_xml(tree, args.max, outdir,
                                dry_run=args.dry_run,
                                verbose=args.verbose)
